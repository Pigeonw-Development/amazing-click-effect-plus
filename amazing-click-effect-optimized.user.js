...
(function() {
    ...
    function clickTriangleEffect() {
        ...
        const MAX_TRACK = 6;   // 尾迹长度优化
        const DOT_TRACK = 3;

        const LONG_PRESS = 800;
        let pressStart = 0, isPress = false;
        let px = 0, py = 0;
        let lastDotSpawn = 0;
        ...
        // Dot参数
        const DOT_R_MIN = 1, DOT_R_MAX = 3;
        const DOT_INIT_SPD = 1.2, DOT_ACCEL = 0.2;
        const DOT_ALPHA_SPD = 0.02, DOT_FADE_BASE = 0.03;
        const DOT_SPAWN_R_MIN = 180, DOT_SPAWN_R_MAX = 280;
        ...
        function createEffect(){
            ...
            if(pressDur >= LONG_PRESS){
                const over = pressDur - LONG_PRESS;
                const scale = Math.min(1, over/SCALE_DUR);
            ...
            for(let i=0;i<createCnt;i++){
                ...
                triAlphaSpd = Math.min(triAlphaSpd + triSpd*2, 50);

                tris.push(new Triangle(px, py, randInt(sizeMin,sizeMax), triSpd, triAlphaSpd/1000));
            }
            // 限制总数量
            if(tris.length > MAX_TRIS) tris.splice(0, tris.length - MAX_TRIS);
        }
        ...
        class Dot {
            ...
            update(){
                if(this.arrived){ this.alpha=0; return; }
                this.addTrack();
                const spdFactor = 1+this.spd*SPEED_ALPHA_FACTOR;
                this.alpha = this.needFade ? Math.max(0,this.alpha-DOT_FADE_BASE*spdFactor)
                                           : Math.min(1,this.alpha+DOT_ALPHA_SPD*spdFactor);
            ...
            draw(){ if(this.alpha<=0||this.arrived) return; this.drawTrail(); ctx.save(); ctx.globalAlpha=this.alpha; ctx.fillStyle=this.color; ctx.beginPath(); ctx.arc(this.x,this.y,this.r,0,Math.PI*2); ctx.closePath(); ctx.fill(); ctx.restore();}
        }
        ...
        function loop(){
            ...
            if(isPress){
                ...
                if(pressDur>=LONG_PRESS){
                    const now=Date.now();
                    if(now-lastDotSpawn>=DOT_INTERVAL && dots.length<MAX_DOTS){
